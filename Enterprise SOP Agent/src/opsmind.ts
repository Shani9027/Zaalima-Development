import type { OpsMindMode, OpsMindRequest, OpsMindResponse } from "./types";
import { searchSimilarDocuments } from "./vectorStore";
import { hasOpenAI } from "./embeddings";
import OpenAI from "openai";

const DEFAULT_MODE: OpsMindMode = "Workflow Assistant";
const openai = process.env.OPENAI_API_KEY ? new OpenAI({ apiKey: process.env.OPENAI_API_KEY }) : null;

function pickMode(request: OpsMindRequest): OpsMindMode {
  if (request.mode) {
    return request.mode;
  }
  ...
  if (question.includes("sop") || question.includes("procedure")) return "SOP Generator";
  if (question.includes("compliance") || question.includes("audit")) return "Compliance Reviewer";
  if (question.includes("incident") || question.includes("outage")) return "Incident Response Guide";
  if (question.includes("onboard")) return "Employee Onboarding Assistant";
  if (question.includes("automation") || question.includes("workflow")) return "Automation Advisor";
  if (question.includes("search") || question.includes("find")) return "Knowledge Search";

  return DEFAULT_MODE;
}

function buildNextQuestions(request: OpsMindRequest): string[] {
  const questions: string[] = [];

  if (!request.department) questions.push("Which department owns this workflow?");
  if (!request.userRole) questions.push("What is the user's role or permission level?");
  if (!request.accessPermissions?.length) questions.push("What access permissions are required?");
  if (!request.complianceRequirements?.length) questions.push("Are there regulatory or policy requirements to enforce?");
  if (!request.organizationHierarchy) questions.push("Who owns the escalation path for this process?");
  if (!request.geographicConstraints) questions.push("Is this affected by any geographic or regulatory constraints?");

  return questions;
}

function buildRisks(request: OpsMindRequest): string[] {
  const risks: string[] = [];

  if (!request.accessPermissions?.length) risks.push("Access permissions are unspecified, which may create least-privilege gaps.");
  if (!request.complianceRequirements?.length) risks.push("Compliance requirements are unspecified, so approval and audit controls may be incomplete.");
  if (!request.geographicConstraints) risks.push("Geographic or regulatory constraints are missing and may affect execution.");
  if (!request.organizationHierarchy) risks.push("Escalation ownership is unclear without an organization hierarchy.");
  if (request.userQuestion.toLowerCase().includes("urgent")) risks.push("Urgent actions should be escalated and validated before execution.");

  return risks;
}

function buildRecommendations(request: OpsMindRequest): string[] {
...
function buildFacts(request: OpsMindRequest): string[] {
  const facts = [
  ...
  if (request.department) facts.push(`Department context provided: ${request.department}.`);
  if (request.userRole) facts.push(`User role context provided: ${request.userRole}.`);

  return facts;
}

function buildSop(request: OpsMindRequest, mode: OpsMindMode): OpsMindResponse["sop"] {
...
function buildRetrievalContext(retrieved: Array<{ source: string; excerpt: string; score: number }>) {
  if (!retrieved.length) {
...
async function buildLLMResponse(request: OpsMindRequest, retrieved: Array<{ source: string; excerpt: string; score: number }>) {
  if (!openai) {
...
function buildFallbackResponse(request: OpsMindRequest, retrieved: Array<{ source: string; excerpt: string; score: number }>) {
...
export async function generateOpsMindResponse(request: OpsMindRequest): Promise<OpsMindResponse> {
  const mode = pickMode(request);
  const nextQuestions = buildNextQuestions(request);

  const retrievedDocuments = await searchSimilarDocuments(request.userQuestion, 5);
  const responseText = hasOpenAI ? await buildLLMResponse(request, retrievedDocuments) : buildFallbackResponse(request, retrievedDocuments);

  const response: OpsMindResponse = {
